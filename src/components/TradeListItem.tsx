...
export function TradeListItem({ 
  ...
  const getConfluenceItems = (): string[] => {
    const items: string[] = [];
    if (!trade.confluenceDetails) return items;
    ...
    <Collapsible open={isExpanded} onOpenChange={handleToggle}>
      ...
      `}>
        ...
        <div className="text-right flex-shrink-0">
          <p className="text-xl">${trade.price.toFixed(2)}</p>
          <div className={`flex items-center gap-1 justify-end text-xs ${isPositive ? 'text-green-500' : 'text-red-500'}`}>
            {isPositive ? <TrendingUp className="w-3 h-3" /> : <TrendingDown className="w-3 h-3" />}
            <span>{isPositive ? '+' : ''}{trade.changePercent.toFixed(2)}%</span>
          </div>
        </div>
      ...
      <CollapsibleContent>
        <div className="px-3 pb-3 space-y-3 border-t border-border/30 pt-3 animate-in fade-in-50 slide-in-from-top-2 duration-200">
              ...
              {confluenceItems.length > 0 && (
                <div className="flex flex-wrap gap-1 pt-1.5 border-t border-border/20">
                  ...
                  {confluenceItems.map((item) => (
                    <Badge key={item} variant="outline" className="text-[10px] h-4 px-1 bg-muted/50">
                      {item}
                    </Badge>
                  ))}
                </div>
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="space-y-0.5 flex-1">
                  ...
                  {trade.warnings?.chopDay && (
                    <p className="text-xs text-red-400">• Chop day</p>
                  )}
          ...
          {trade.confluenceDetails && trade.confluenceScore !== undefined && (
            <div>
          ...
          {trade.riskReward && (
            ...
            {trade.patientCandle && (
                <div className="p-2 rounded bg-muted/20">
                  ...
                  <p className="text-[10px]">{trade.patientCandle.isContained ? "Contained ✓" : "Invalid ✗"}</p>
                </div>
              )}
            </div>
          )}
          ...
          {hasOptions && trade.optionsContract && trade.position && (
            <div className="p-2 rounded-lg bg-gradient-to-br from-indigo-500/10 to-purple-500/10 border border-indigo-500/20">
              <div className="flex items-center justify-between mb-1.5">
                ...
                <Badge variant="outline" className="text-[10px] py-0 px-1.5 bg-indigo-500/10 text-indigo-400 border-indigo-500/20">
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                {trade.tradeState !== "LOADED" && (
                  <p className={`text-xs ${trade.position.totalPL >= 0 ? 'text-green-500' : 'text-red-500'}`}>
