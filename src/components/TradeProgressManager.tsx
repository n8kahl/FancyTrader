...
interface TradeProgressManagerProps {
  ...
  onSendAlert: (type: AlertType, message: string, data?: any) => void;
  onUpdateTradeState: (tradeId: string, newState: "LOADED" | "ENTERED" | "ACTIVE" | "CLOSED") => void;
...
export function TradeProgressManager({
  ...
  const handleActionClick = (action: AlertType) => {
    ...
    const message = template.defaultMessage
      ...
      .replace("{conviction}", trade.conviction)
      .replace("{target}", contract.breakEven.toFixed(2))
      .replace("{stop}", trade.stop.toFixed(2))
  ...
  const handleSendAlert = () => {
    ...
    // Send the alert
    onSendAlert(selectedAction, finalMessage, actionData);
              ...
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-mono">
                    {trade.symbol} ${contract.strike}{contract.type === "CALL" ? "C" : "P"}
                  </p>
            ...
            <div className="p-4 rounded-lg border-2 border-border/50 bg-gradient-to-br from-card/50 to-muted/20">
              ...
              <div className="grid grid-cols-2 gap-4 mb-3">
                ...
                <div>
                  ...
                  <p className={`text-2xl ${profitColor}`}>
                    {position.totalPLPercent > 0 ? '+' : ''}{position.totalPLPercent.toFixed(1)}%
                  </p>
                </div>
              </div>
                  ...
                  {onDismissTrade && (
                    <div className="pt-4 mt-4 border-t border-border/30">
                      <Button
                        ...
                        onClick={() => {
                          if (confirm(`Are you sure you want to dismiss this trade? This will remove ${trade.symbol} from your active trades.`)) {
          ...
          <div className="flex flex-col gap-4 overflow-hidden">
            {selectedAction ? (
              <>
                ...
                <div className="flex-1 overflow-hidden flex flex-col">
                  <Label htmlFor="message" className="mb-2">Message Preview</Label>
                  <Textarea
                    id="message"
                    value={customMessage}
