...
export function PatientCandleVisualizer({
  ...
  const isContained = direction === "LONG" 
  ...
  const pcIsBullish = pcClose > pcOpen;
    ...
    <div className={`${compact ? 'p-2' : 'p-4'} rounded-lg bg-muted/20 border border-border/30`}>
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex flex-col gap-2 text-[10px] text-muted-foreground min-w-[70px]">
          ...
          <div className="flex items-center gap-1">
            <div className={`w-2 h-2 rounded-sm ${direction === "LONG" ? 'bg-green-500' : 'bg-red-500'}`} />
            <span>PC</span>
          </div>
        ...
        <div className="flex-1 relative" style={{ height: compact ? '120px' : '160px' }}>
          ...
          <svg className="w-full h-full" viewBox="0 0 200 100" preserveAspectRatio="none">
            {/* Background grid */}
            <line x1="0" y1="25" x2="200" y2="25" stroke="currentColor" strokeWidth="0.2" className="text-border" strokeDasharray="2,2" />
            <line x1="0" y1="50" x2="200" y2="50" stroke="currentColor" strokeWidth="0.2" className="text-border" strokeDasharray="2,2" />
            <line x1="0" y1="75" x2="200" y2="75" stroke="currentColor" strokeWidth="0.2" className="text-border" strokeDasharray="2,2" />
            ...
            {/* Labels */}
            <text x="60" y="97" fontSize="4" fill="currentColor" className="text-muted-foreground" textAnchor="middle">
              Prior
            </text>
            <text x="130" y="97" fontSize="4" fill={direction === "LONG" ? "#22c55e" : "#ef4444"} textAnchor="middle">
              PC
            </text>
          </svg>
        </div>
        ...
        {isContained ? (
          <div className="flex items-start gap-2">
            ...
            <div className="space-y-1.5">
              <p>
                <span className="opacity-80">✓ Containment Rule Met:</span> PC is fully inside prior candle range
              </p>
              <p className="opacity-70">
                <span className="opacity-90">Entry Trigger:</span> Price breaks {direction === "LONG" ? "above" : "below"} PC {direction === "LONG" ? "high" : "low"} (${direction === "LONG" ? pcHigh.toFixed(2) : pcLow.toFixed(2)})
              </p>
              <p className="opacity-70">
                <span className="opacity-90">Stop:</span> ${stop.toFixed(2)} (other side of PC at ${direction === "LONG" ? pcLow.toFixed(2) : pcHigh.toFixed(2)})
              </p>
            </div>
          </div>
        ) : (
          <div className="flex items-start gap-2">
            ...
            <div>
              ...
              <p className="opacity-70">
                {pcHigh > priorHigh && "PC high ($" + pcHigh.toFixed(2) + ") exceeds prior high ($" + priorHigh.toFixed(2) + ")"}
                {pcLow < priorLow && "PC low ($" + pcLow.toFixed(2) + ") below prior low ($" + priorLow.toFixed(2) + ")"}
              </p>
            </div>
          </div>
        )}
