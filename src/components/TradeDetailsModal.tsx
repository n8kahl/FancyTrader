...
import { Send, TrendingUp, TrendingDown, CheckCircle2, XCircle, AlertTriangle, Clock, DollarSign, History } from "lucide-react";
...
import { calculateConfluenceScore, getStrengthColor, formatConfluenceValue } from "../types/confluence";
...
export function TradeDetailsModal({ trade, isOpen, onClose, onSendAlert }: TradeDetailsModalProps) {
  ...
  const priceData = trade.priceData || generateMockCandles(
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh]">
        <DialogHeader>
          ...
          <DialogDescription>
            {trade.setup} setup with {trade.conviction} conviction
          </DialogDescription>
        </DialogHeader>
        ...
        <ScrollArea className="max-h-[calc(90vh-120px)] pr-4">
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-3xl">${trade.price.toFixed(2)}</p>
            ...
            <div className="flex flex-col gap-2 items-end">
              <Badge 
                variant="outline" 
                className={
                  trade.status === "ACTIVE" ? "text-green-500 bg-green-500/10 border-green-500/20" :
                    ...
                    {trade.warnings.sma200Headwind && (
                      <div className="flex items-start gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-red-400/70">200 SMA is blocking the trade direction - skip for poor R:R</p>
                        </div>
                      </div>
                    )}
                    {trade.warnings.chopDay && (
                      <div className="flex items-start gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-red-400/70">ORB indicates consolidation - treat as study day</p>
                        </div>
                      </div>
                    )}
                    {trade.warnings.preVWAPTime && (
                      <div className="flex items-start gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-red-400/70">VWAP strategies activate after 10:00 ET</p>
                        </div>
                      </div>
                    )}
                    {trade.warnings.poorRiskReward && (
                      <div className="flex items-start gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-red-400/70">R:R below minimum 1:2 threshold - prefer 1:3+</p>
                        </div>
                      </div>
                    )}
                    {trade.warnings.pcInvalid && (
                      <div className="flex items-start gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-red-400/70">PC wick breaches prior candle range - invalidates setup</p>
                        </div>
                      </div>
                    )}
                  </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="p-4 rounded-lg bg-muted/30 border border-border/30">
              ...
              {strategyInstructions && (
                <div className="text-xs text-muted-foreground space-y-1 pt-2 border-t border-border/20 mt-2">
                  ...
                  <p>• Required confluence: {strategyInstructions.requiredConfluence.join(", ")}</p>
            ...
            <div className="p-4 rounded-lg bg-muted/30 border border-border/30">
              ...
              <Badge 
                variant="outline" 
                className={
                  trade.conviction === "HIGH" ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/20" :
                  trade.conviction === "MEDIUM" ? "bg-amber-500/10 text-amber-500 border-amber-500/20" :
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="p-4 rounded-lg bg-green-500/10 border border-green-500/20">
              ...
              <p className="text-xl text-green-500">${trade.entry.toFixed(2)}</p>
              <p className="text-xs text-muted-foreground mt-1">
                {Math.abs(trade.price - trade.entry).toFixed(2)} pts away
              </p>
            </div>
            <div className="p-4 rounded-lg bg-blue-500/10 border border-blue-500/20">
              <p className="text-sm text-muted-foreground mb-1">Target</p>
              <p className="text-xl text-blue-500">${trade.target.toFixed(2)}</p>
              <p className="text-xs text-muted-foreground mt-1">
                {Math.abs(trade.target - trade.entry).toFixed(2)} pts upside
              </p>
            </div>
            <div className="p-4 rounded-lg bg-red-500/10 border border-red-500/20">
              <p className="text-sm text-muted-foreground mb-1">Stop Loss</p>
              <p className="text-xl text-red-500">${trade.stop.toFixed(2)}</p>
              <p className="text-xs text-muted-foreground mt-1">
                {Math.abs(trade.entry - trade.stop).toFixed(2)} pts risk
              </p>
            </div>
          </div>
          ...
          <div className="p-4 rounded-lg bg-indigo-500/10 border border-indigo-500/20">
            ...
            <p className="text-2xl text-indigo-400">{trade.riskReward}</p>
            <p className="text-xs text-muted-foreground mt-1">
                  ...
                  <div>
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Range: ${(trade.patientCandle.priorHigh - trade.patientCandle.priorLow).toFixed(2)}
                      </p>
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Range: ${(trade.patientCandle.pcHigh - trade.patientCandle.pcLow).toFixed(2)}
                      </p>
                    </div>
                  </div>
                </div>
                <div className={`mt-3 p-3 rounded-lg border ${
                  ...
                  {trade.patientCandle.isContained ? (
                    <div className="flex items-start gap-2 text-xs">
                      ...
                      <div>
                        <p className="mb-1">✓ Containment Rule Met: PC fully inside prior candle</p>
                        <p className="opacity-70">
                          Entry Trigger: Break {trade.patientCandle.direction === "LONG" ? "above" : "below"} ${trade.patientCandle.direction === "LONG" ? trade.patientCandle.pcHigh.toFixed(2) : trade.patientCandle.pcLow.toFixed(2)}
                        </p>
                      </div>
                    </div>
                  ) : (
                    <div className="flex items-start gap-2 text-xs">
                      ...
                      <p>✗ Invalid Setup: PC wick breaches prior candle range - invalidates setup</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  ...
                  {trade.tradeState !== "LOADED" && (
                    <div className="p-4 rounded-lg border border-border/50 bg-card/30">
                      ...
                      <p className={`text-3xl mb-1 ${trade.position.totalPL >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                        {trade.position.totalPL >= 0 ? '+' : ''}${Math.abs(trade.position.totalPL).toFixed(0)}
                      </p>
                      <p className={`text-sm mb-3 ${trade.position.totalPLPercent >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                ...
                <div className="grid grid-cols-3 gap-3 text-sm">
                  ...
                  <div className="p-3 rounded-lg bg-muted/20 border border-border/30">
                    ...
                    <p className={`text-lg ${trade.position.unrealizedPL >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                      ${Math.abs(trade.position.unrealizedPL).toFixed(0)}
                    </p>
                  </div>
                  <div className="p-3 rounded-lg bg-muted/20 border border-border/30">
                    <p className="text-muted-foreground mb-1">Realized P&L</p>
                    <p className={`text-lg ${trade.position.realizedPL >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                      ...
                      >
                        ...
                        {alert.profitLoss !== undefined && (
                          <div className="pt-2 border-t border-border/30 text-xs">
                            ...
                            <p className={alert.profitLoss >= 0 ? "text-green-500" : "text-red-500"}>
                              ${alert.profitLoss.toFixed(0)} ({alert.profitLossPercent?.toFixed(1)}%)
                            </p>
                          </div>
                        )}
                      </div>
              ...
              {trade.confluenceFactors && trade.confluenceFactors.length > 0 ? (
                trade.confluenceFactors.map((factor) => (
                  ...
                  >
                    <div className="flex items-center gap-2 mb-2">
                      {factor.present ? (
                        <CheckCircle2 className={`w-4 h-4 ${
                          factor.strength === "HIGH" ? "text-emerald-400" :
                          factor.strength === "MEDIUM" ? "text-amber-400" :
                          "text-gray-400"
                        }`} />
                      ) : (
                    ...
                    {factor.present && (
                      <div className="mt-2">
                        <Badge variant="outline" className={`text-xs ${getStrengthColor(factor.strength)}`}>
                          {factor.strength}
                        </Badge>
                      </div>
                    )}
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="text-xs">
                    <p className="text-indigo-400 mb-1">
                      Active Confluence Factors ({activeConfluence.length}/{trade.confluenceFactors.length})
                    </p>
                    <p className="text-muted-foreground">
                      {activeConfluence.map(c => `${c.factor}: ${formatConfluenceValue(c.value)}`).join(' • ')}
                    </p>
                  </div>
                </div>
              </div>
