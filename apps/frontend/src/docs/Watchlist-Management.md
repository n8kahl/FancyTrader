...
```typescript
const activeSymbols = watchlist.filter((s) => s.isActive).map((s) => s.symbol);
// ["AAPL", "MSFT", "GOOGL", ...]
```
...
_The Watchlist Manager gives admins full control over which symbols are monitored, making the system flexible for different trading styles and market conditions._
