...
```typescript
const { isConnected, trades, subscribeToSymbols, refreshSetups } = useBackendConnection();
```
...
```typescript
const { isConnected, trades, subscribeToSymbols } = useBackendConnection(!useMockData);
...
```typescript
const TradeDetailsModal = lazy(() => import("./components/TradeDetailsModal"));
```
