    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="max-w-screen-2xl mx-auto px-4 py-6">
        <div className="w-full space-y-4">
          {/* Backend Setup Guide - Only show when diagnostics enabled and backend unreachable */}
          {DIAGNOSTICS_ENABLED && !useMockData && !isConnected && !isLoading && backendError && (
            <BackendSetupGuide
                ...
                <>
                  {viewMode === "grid" ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                      {filteredTrades.map((trade) => renderTradeCard(trade))}
                    </div>
                  ) : (
                    <div className="space-y-2">
                      {filteredTrades.map((trade) => renderTradeListItem(trade))}
                    </div>
                  )}
                </>
              )}
      ...
      {tradeProgressTrade?.optionsContract && tradeProgressTrade.position && (
      ...
      {DIAGNOSTICS_ENABLED && <DiagnosticPanel />}
