...
export function EntryProgressIndicator({
  trade,
  progress,
  compact = false,
}: EntryProgressIndicatorProps) {
  // Define entry criteria based on trade
  const criteria: EntryCriterion[] = [];

  const confluenceScore = trade.confluenceScore ?? 0;
  const warnings = Object.values(trade.warnings ?? {});
  const hasWarnings = warnings.some((v) => v === true);
  const marketPhase = trade.marketPhase;

  // 1. Confluence Check
  const hasMinimalConfluence = confluenceScore >= 6;
  criteria.push({
    id: "confluence",
  ...
  const hasCorrectPhase = marketPhase && marketPhase !== "PRE_MARKET";
  criteria.push({
    id: "market_phase",
    label: "Market phase valid",
    isMet: hasCorrectPhase || !marketPhase,
  });
    ...
    <div className="mb-4">
          ...
          {[0, 25, 50, 75, 100].map((milestone) => (
            <div
              ...
              className={cn(
                ...
                completionPercent >= milestone ? "bg-white shadow-md" : "bg-muted-foreground/30"
              )}
      ...
      <div className="grid grid-cols-2 gap-x-3 gap-y-1.5">
        {criteria.map((criterion) => (
          <div key={criterion.id} className="flex items-start gap-1.5">
            {criterion.isMet ? (
