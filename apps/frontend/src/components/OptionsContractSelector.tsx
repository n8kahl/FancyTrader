...
import type { Trade, OptionsContract } from "@/types/trade";
...
function ContractCard({
  ...
  const projectedProfit = contract.projectedProfit ?? 0;
  const projectedPercent = contract.projectedProfitPercent ?? 0;
  ...
  return (
    ...
    >
      {isRecommended && (
        <Badge className="absolute -top-2 -right-2 bg-indigo-600">Recommended</Badge>
      )}
      ...
      <div className="flex items-start justify-between mb-3">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <Badge
              variant={contract.type === "CALL" ? "default" : "secondary"}
              className="text-[10px]"
            >
              {contract.type}
            </Badge>
        ...
        >
          <div className="flex items-center justify-between text-xs">
            ...
            <div className="flex items-center gap-1">
              ...
              <span>${Math.abs(projectedProfit).toFixed(0)}</span>
              <span>
                ({projectedPercent > 0 ? "+" : ""}
                {projectedPercent}%)
              </span>
...
export function OptionsContractSelector({
  ...
  const targetPrice = trade.target ?? trade.targets?.[0] ?? price;
  const stopPrice = trade.stop ?? trade.stopLoss ?? price;
  const conviction = trade.conviction ?? "MEDIUM";
  const direction: "LONG" | "SHORT" = trade.patientCandle?.direction ?? trade.direction ?? "LONG";

  const { itm, otm } = useMemo(
    () => generateContracts(trade.symbol, price, targetPrice, conviction, direction),
    [trade.symbol, price, targetPrice, conviction, direction]
  );

  // Recommend based on conviction
  const getRecommendedIndex = (): number => {
    if (conviction === "HIGH") return 0;
    if (conviction === "MEDIUM") return 1;
    return 2;
  };

  const recommendedOTM = Math.min(getRecommendedIndex(), Math.max(otm.length - 1, 0));
      ...
      <DialogContent className="max-w-4xl max-h-[85vh]">
        ...
        <div className="grid grid-cols-3 gap-3 p-3 rounded-lg bg-muted/30 border border-border/30">
          ...
          <div>
            ...
            <p className="text-lg text-green-500">${targetPrice.toFixed(2)}</p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground">Stop</p>
            <p className="text-lg text-red-500">${stopPrice.toFixed(2)}</p>
          </div>
        </div>
