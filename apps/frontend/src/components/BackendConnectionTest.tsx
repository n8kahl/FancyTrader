...
export function BackendConnectionTest() {
  ...
  const getStatusIcon = (status: TestResult["status"]): ReactNode => {
    switch (status) {
      case "success":
  ...
  const getStatusBadge = (status: TestResult["status"]): ReactNode => {
    switch (status) {
      case "success":
        return (
  ...
  return (
    <Card className="p-6 space-y-6">
      ...
      <Button onClick={runTests} disabled={isTestingRunning} className="w-full">
        {isTestingRunning ? (
          <>
