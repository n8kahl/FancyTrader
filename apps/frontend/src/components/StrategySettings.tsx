...
export function StrategySettings({
  ...
  const getCategoryIcon = (category: StrategyCategory): JSX.Element | null => {
    switch (category) {
      case "KCU_LTP":
  ...
  const renderStrategyList = (strategies: StrategyDefinition[]): JSX.Element[] => {
    // Group by category
                ...
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap gap-1">
                    ...
                    {strategy.validationRules?.minimumRR && (
                      <Badge
                        variant="outline"
                        className="text-[10px] py-0 bg-green-500/10 text-green-500 border-green-500/20"
                      >
          ...
          <Tabs defaultValue="all" className="w-full">
            <TabsList className="grid w-full grid-cols-4">
              ...
              <TabsTrigger value="kcu" className="text-xs">
                KCU ({KCU_STRATEGIES.filter((s) => isStrategyEnabled(s.id)).length}/
                {KCU_STRATEGIES.length})
              </TabsTrigger>
              <TabsTrigger value="universal" className="text-xs">
                Universal ({UNIVERSAL_STRATEGIES.filter((s) => isStrategyEnabled(s.id)).length}/
                {UNIVERSAL_STRATEGIES.length})
              </TabsTrigger>
              <TabsTrigger value="options" className="text-xs">
                Options ({OPTIONS_STRATEGIES.filter((s) => isStrategyEnabled(s.id)).length}/
                {OPTIONS_STRATEGIES.length})
              </TabsTrigger>
            </TabsList>
